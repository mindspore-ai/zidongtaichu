...
class COCO:
    def __init__(self, annotation_file=None, cut=True):
        """
        ...
        if not annotation_file == None:
            ...
            self.createIndex(cut)

    def createIndex(self, cut):
        # create index
        ...
        images = {vi['image_id']: {} for vi in self.dataset['annotations']}
        print ('index created!')
    ...
    def loadRes(self, resFile, cut=True):
        ...
        for ann in anns:
            ...
            #     p = ' '.join(w)
            # else:
            #     p = ''.join([w for w in ann['caption'].strip().replace('。','')])
            p = ''.join([w for w in ann['caption'].strip().replace('。','')])
            ann['caption'] = p
