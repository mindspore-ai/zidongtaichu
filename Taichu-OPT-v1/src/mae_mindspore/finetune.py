...
from src.modules import VitTrainOneStepCell
from src.helper import parse_with_config, download
from src.monitor import StateMonitor, UploadCheckpoint
...
# 再配置ak sk等信息
mox.file.set_auth(ak='GL1XBYLXHMCJYPWVWTP7',
                  sk='m7vIhvIQ9W27So3jCVzuFTyqru67BQuK7xYp10lG',
...
def main(args):
    ...
    # define optimizer
    optimizer = nn.AdamWeightDecay(net.trainable_params(), learning_rate=lr_schedule, weight_decay=args.weight_decay, beta1=args.beta1, beta2=args.beta2)

    # load finetune ckpt
    if args.finetune_ckpt:
        params_dict = download(args, local_rank)
        ...
        args.logger.info(f"===============opt_not_load================{opt_not_load}")
    ...
    net_with_train = VitTrainOneStepCell(net_with_loss, optimizer, args.loss_scale)
    ...
    if local_rank == 0:
        ...
        ckpoint_cb = ModelCheckpoint(prefix=args.prefix,
                                     ...
                                     config=config_ck)
        obs_cb = UploadCheckpoint(args.ckpt_save_dir, args.obs_dir, save_ckpt_feq)
        callback += [ckpoint_cb, obs_cb]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--config', default="/home/work/user-job-dir/mae_mindspore/config/vit-large-p16.yaml",
