...
do
    ...
    rm -rf ./train_vqa_log$i
    mkdir ./train_vqa_log$i
    echo "start training for rank $RANK_ID, device $DEVICE_ID"
    cd ./train_vqa_log$i ||exit
    env > env.log
    cd ..
    python -u src/scripts/finetune_vqa.py \
        --config config/vqa/finetune_vqa_base.json \
        --use_parallel "true" > ./train_vqa_log$i/log 2>&1
done