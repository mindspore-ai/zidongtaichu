...
do
    ...
    rm -rf ./train_caption_log$i
    mkdir ./train_caption_log$i
    echo "start training for rank $RANK_ID, device $DEVICE_ID"
    cd ./train_caption_log$i ||exit
    env > env.log
    cd ..
    python -u src/scripts/finetune_caption.py \
        --config config/caption/finetune_caption_base.json \
        --use_parallel "true" > ./train_caption_log$i/log 2>&1
done