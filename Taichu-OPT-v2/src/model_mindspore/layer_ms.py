...
from src.model_mindspore.parallel_transformer import LayerNorm2
...
class RegionFeatureRegression(nn.Cell):
    ...
    def construct(self, input_):
        ...
        # output = self.linear(hidden, self.t(self.weight, (1, 0)), self.bias)
...
class BertLMPredictionHead(nn.Cell):
    ...
    def __init__(self, config, bert_model_embedding_weights, parallel_config):
        ...
        # self.bias = Parameter(
        #     Tensor(np.zeros(bert_model_embedding_weights.shape[0]), mstype.float32))
