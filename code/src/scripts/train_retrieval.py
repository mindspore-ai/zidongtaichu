...
from data import create_dataset, get_batch_data
...
from freeze_model_utils import get_freeze_layers, freeze_model
...
def init_env(opts):
    ...
    profiling_path = os.path.join(opts.output_path, f'cache/{local_rank}-graphs/')
...
def save_eval_result(result, opts):
    save_path = os.path.join(opts.output_path, "eval_result.json")
...
def main(opts):
    ...
    if opts.eval_only:
        ...
        if opts.ckpt_path:
            load_ckpt(net_without_loss, os.path.join(opts.ckpt_path, opts.ckpt_file))
