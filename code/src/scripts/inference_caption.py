...
def inference_directory(net, image_dir, vocab, infer_result_path):
...
def inference_list_json(net, image_dir, list_file, vocab, infer_result_path):
...
def load_ckpt(net, ckpt_path):
    if not ckpt_path:
        return
    print(f"start loading ckpt:{ckpt_path}")
    ...
    print(f"end loading ckpt:{ckpt_path}")


def main(opts):
    if opts.ckpt_path:
        opts.ckpt_path = os.path.join(opts.ckpt_path, opts.ckpt_file)

    infer_result_path = join(opts.output_path + "/infer_result.json")
    print("init environment")
    device_id = int(os.getenv('DEVICE_ID') if os.getenv('DEVICE_ID') else 0)
    context.set_context(mode=context.GRAPH_MODE, device_target="Ascend", device_id=device_id)

    print("load network")
    net = UniterThreeForPretrainingForCapFinetuneInf(opts.model_config, IMG_DIM, AUDIO_DIM, full_batch=False,
                                                     beam_width=opts.beam_width)
    load_ckpt(net, opts.ckpt_path)

    opts.vocab_path = os.path.join(opts.data_path, opts.vocab_path)
    vocab = json.load(open(opts.vocab_path))
    if os.path.isfile(opts.inference_list):
        inference_list_json(net, opts.data_path, opts.inference_list, vocab, infer_result_path)
    elif os.path.isdir(opts.data_path):
        inference_directory(net, opts.data_path, vocab, infer_result_path)

    print('infer_config_caption.yaml task ends')


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    abs_path = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), "../.."))
    model_config_real_path = os.path.join(abs_path, "config/caption/cross_modal_encoder_base.json")
    parser.add_argument('--model_config', default=model_config_real_path, type=str, help='model config file')
    parser.add_argument('--output_path', required=True, default="", type=str, help='output path')
    parser.add_argument('--ckpt_path', required=True, default="", type=str, help='check point file path')
    parser.add_argument('--ckpt_file', required=False, default="OPT_cap-10_7076.ckpt", type=str,
                        help='check point file')
    parser.add_argument('--beam_width', default="4", type=int, help='beam search width')
    parser.add_argument('--vocab_path', default='ids_to_tokens_zh.json', type=str, help='vocab file')
