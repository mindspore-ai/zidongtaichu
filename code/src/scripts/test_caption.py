...
def main(opts):
    ...
    res_dir = join(opts.output_path, 'eval')
    eval_file_name = "_result.json"
    eval_result_path = join(res_dir + eval_file_name)
    if not os.path.exists(res_dir):
        os.mkdir(res_dir)
    res_name = opts.ckpt_path.split('/')[-1].replace(".ckpt", ".json")
    ...
    load_ckpt(net_without_loss, opts.ckpt_path.strip())

    validate_td(net_without_loss, test_loader, opts, res_path, eval_result_path)


def validate_td(model, test_loader, opts, res_path, eval_result_path):
    """
     validate_td
    """
    ...
    eval_result = compute_metric(opts.caption_eval_gt, res_path, opts.cut)
    json.dump(eval_result,open(res_path.replace('.json','_metric.json'),'w'))
    json.dump(eval_result, open(eval_result_path, 'w'))
    print(eval_result)
...
if __name__ == "__main__":
    ...
    real_path = os.path.join(abs_path, "config/caption/ft_cap_base.json")
    ...
    parser.add_argument('--ckpt_path', default="", type=str, help='use txt out')
    parser.add_argument('--ckpt_file', default="OPT_cap-10_7076.ckpt", type=str, help="ckpt file name")
    ...
    parser.add_argument('--train_url', required=False, default=None, help='Location of data.')
