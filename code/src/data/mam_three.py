...
def _get_audio_feat_target(audio_feat, audio_masks):
    # audio_masks_ext = audio_masks.unsqueeze(-1).expand_as(audio_feat)  # (n, m, d)
...
class MafrThreeDataset(TxtImgAudioDataset):
    """MafrThreeDataset"""
    ...
    def __getitem__(self, i):
        ...
        img_feat, img_pos_feat, _, image = self._get_img_feat(
            example['img_fname'])
        ...
        return (input_ids, img_feat, img_pos_feat,
                attn_masks,
                audio_feat, audio_mask, audio_mask_tgt, image)
...
def mafrThree_collate(inputs):
    ...
    """
    (input_ids, img_feats, img_pos_feats, attn_masks, audio_feats, audio_masks, audio_mask_tgts, images) = map(list,
                                                                                                       unzip(inputs))
    ...
    #audio_mask_tgt_mask = np.stack((audio_mask_tgt == True).nonzero(), 1)
    ...
    # neg_index = audio_masks.view(-1).nonzero()
