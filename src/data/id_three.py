...
from src.data.data_three import pad_tensors_pos, pad_tensors, stack_images
from src.data.data_three import TxtImgAudioDataset, get_gather_index_three
from src.data.mlm_three import random_word_with_all
from src.data.mrm_three import _get_img_mask_with_all
from src.data.mam_three import _get_audio_mask
...
def _mask_audio_feat(audio_feat, audio_masks):
    if audio_masks is None or audio_masks[0] is None:
        return audio_feat
    # audio_masks_ext = audio_masks.unsqueeze(-1).expand_as(audio_feat)
    audio_masks_ext = np.broadcast_to(np.expand_dims(audio_masks, -1), audio_feat.shape)
    # audio_feat_masked = audio_feat.data.masked_fill(audio_masks_ext, 0)
    audio_feat[audio_masks_ext] = 0
    return audio_feat


class IdThreeDataset(TxtImgAudioDataset):
    """
    IdThreeDataset
    """
    ...
    def __getitem__(self, i):
        ...
        return (ids, input_ids, attn_masks, txt_labels, img_feat, img_pos_feat, img_mask, audio_feat,
                ...
                img_token_inputs, img_token_gts, img_token_masks, image)
...
def idThree_collate(inputs):
    ...
    (ids, input_ids, attn_masks, txt_labels, img_feats, img_pos_feats, img_masks, audio_feats,
     ...
     img_token_inputs, img_token_gts, img_token_masks, images) = map(list, unzip(inputs))
    ...
    # img tokens
    img_token_inputs = pad_sequence(img_token_inputs, batch_first=True, padding_value=0, max_lens=config.IMG_TOKEN_LEN)
    img_token_gts = pad_sequence(img_token_gts, batch_first=True, padding_value=0, max_lens=config.IMG_TOKEN_LEN)
    img_token_masks = pad_sequence(img_token_masks, batch_first=True, padding_value=0, max_lens=config.IMG_TOKEN_LEN).astype(np.float32)
